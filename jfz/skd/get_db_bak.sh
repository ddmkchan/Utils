...
wget_cmd="wget $FILE_LIST/$yes_date/filelist.txt -O $list_file"
...
do
	...
	curl -k --get --data "session.id=${session}" --data "ajax=executeFlow" --data "project=jfz_rawdata_to_hive" --data "flow=to_hive_single" --data "flowOverride[param.tableName]=${tableName}" --data "flowOverride[param.database]=${database}" --data "flowOverride[param.localDataPath]=${localDataPath}" https://localhost:8443/executor
